...
class QRcode {
	...
	public function png($url, $outfile = false, $size = 5, $evel = 'H') {
		...
		if (!$outfile) {
			$outfile = $this->cache_dir . '/' . $this->gName() . '.png';
		}
	...
	public function logo($logo) {
		...
		$this->outfile = $this->cache_dir . '/' . $this->gName() . '.png';
	...
	public function gName($prefix = '') {
		mt_srand((double)microtime(true) * 10000);
		return strtolower(md5(uniqid($prefix . rand(), true)));
	}
